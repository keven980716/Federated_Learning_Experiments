# This file is based on the commit by Elliebababa
import argparse
import logging
import os
import json
...
if __name__ == "__main__":
    ...
    if process_id == 0:
        ...
        for i in range(args.client_num_per_round):
            worker_idx = i + 1
            worker_file = '~/fednlp_data/distributed/{}_distributed/{}.json'.format(args.dataset, worker_idx)
            with open(worker_file, 'r') as f:
                client_list = json.load(f)
                worker_lists.append(client_list)
        preprocessed_sampling_lists = list(
            map(list, zip(*worker_lists)))  # return [[round1 clients list], [round2 clients list], ...]
    else:
