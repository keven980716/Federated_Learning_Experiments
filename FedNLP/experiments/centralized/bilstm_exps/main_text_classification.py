...
def load_data(args, dataset_name):
    data_loader = None
    # there are bugs in original code, data_loader() missing, which is in obsolete dir
    if dataset_name == "20news":
        logging.info("load_data. dataset_name = %s" % dataset_name)
        data_loader = data.raw_data_loader.obsolete.news_20.data_loader. \
        ...
        logging.info("load_data. dataset_name = %s" % dataset_name)
        data_loader = data.raw_data_loader.obsolete.AGNews.data_loader. \
            ClientDataLoader(os.path.abspath(args.data_file), os.path.abspath(args.partition_file),
                             partition_method=args.partition_method, tokenize=True)
