...
# For now, we only write the code for classification tasks
...
def label_uniform_process(label_vocab, label_assignment, client_num,
                          data_length):
    """
    params
    -------------------------------------------------------------------
    ...
    client_length = 0
    print("client_num", client_num)
    idxs = np.random.permutation(data_length)
    batch_idxs = np.array_split(idxs, client_num)
    for j in range(client_num):
        partition_result[j] = batch_idxs[j]
...
def main():
    ...
    parser.add_argument("--seed", type=int, metavar="RS", help="random seed", default=12)
    ...
    partition_result_train = label_uniform_process(label_vocab_train,
                                                label_assignment_train,
                                                client_num,
                                                train_length)
    print("start test data processing")
    partition_result_test = label_uniform_process(label_vocab_test,
                                               label_assignment_test,
                                               client_num, test_length)
    ...
    if ("/uniform" + flag_str + "_clients=%.1f" %
        (args.client_number) in partition):
        del partition["/uniform_" + flag_str + "_clients=%.1f" %
                      (args.client_number)]
    if ("/uniform_" + flag_str + "_clients=%d" %
        (args.client_number) in partition):
        del partition["/uniform_" + flag_str + "_clients=%d" %
                      (args.client_number)]

    partition["/uniform_" + flag_str + "_clients=%d" %
              (args.client_number) + "/n_clients"] = client_num
    partition["/uniform_" + flag_str + "_clients=%d" %
              (args.client_number) + "/alpha"] = 'uniform'
    for partition_id in range(client_num):
        train = partition_result_train[partition_id]
        test = partition_result_test[partition_id]
        train_path = ("/uniform_" + flag_str + "_clients=%d" %
                      (args.client_number) + "/partition_data/" +
                      str(partition_id) + "/train/")
        test_path = ("/uniform_" + flag_str + "_clients=%d" %
                     (args.client_number) + "/partition_data/" +
