...
class TextClassificationSCAFFOLDTrainer:
    ...
    def train_model(self, device=None, global_control_variate=0, local_control_variate=0):
        if not device:
            device = self.device
        ...
        if global_control_variate == 0:
            local_control_variate = 0
        elif local_control_variate == 0:
            local_control_variate = global_control_variate.to(device)
        else:
            for k in local_control_variate.state_dict().keys():
                if 'weight' in k or 'bias' in k:
                    #local_control_variate.state_dict()[k].data = global_control_variate.state_dict()[k].data - local_control_variate.state_dict()[k].data
                    local_control_variate.state_dict()[k].data -= 2 * local_control_variate.state_dict()[k].data
                    local_control_variate.state_dict()[k].data += global_control_variate.state_dict()[k].data
            local_control_variate.to(device)
        new_local_control_variate = None
        ...
        for epoch in range(0, self.args.epochs):
            ...
            for batch_idx, batch in enumerate(self.train_dl):
                ...
                if (batch_idx + 1) % self.args.gradient_accumulation_steps == 0:
                    ...
                    if new_local_control_variate is None:
                        new_local_control_variate = copy.deepcopy(original_model)
                        for k, v in self.model.named_parameters():
                            if 'weight' in k or 'bias' in k:
                                # print(new_local_control_variate.state_dict()[k].norm(), v.grad.norm())
                                new_local_control_variate.state_dict()[k].data -= new_local_control_variate.state_dict()[
                                    k].data
                                new_local_control_variate.state_dict()[k].data += v.grad.data.to('cpu')
                                # print(new_local_control_variate.state_dict()[k].norm())
                    else:
                        for k, v in self.model.named_parameters():
                            if 'weight' in k or 'bias' in k:
                                new_local_control_variate.state_dict()[k].data += v.grad.data.to('cpu')

                    # add control variate to gradient
                    if local_control_variate != 0:
                        for k, v in self.model.named_parameters():
                            if 'weight' in k or 'bias' in k:
                                #print(k, v.grad.norm(), local_control_variate.state_dict()[k].norm())
                                v.grad.data += local_control_variate.state_dict()[k].data
                                #print(v.grad.norm())
                            else:
                                print(k)
                    ...
                    #     for k, v in self.model.named_parameters():
                    #         # print(new_local_control_variate.state_dict()[k].norm(), v.grad.norm())
                    #         new_local_control_variate.state_dict()[k].data -= new_local_control_variate.state_dict()[k].data
                    #         new_local_control_variate.state_dict()[k].data += v.grad.data.to('cpu')
                    #         # print(new_local_control_variate.state_dict()[k].norm())
                    # else:
                    #     for k, v in self.model.named_parameters():
        ...
        # local_control_variate = copy.deepcopy(original_model)
        ############
        # update local control variate
        for k in new_local_control_variate.state_dict().keys():
            new_local_control_variate.state_dict()[k].data /= global_step
        #    local_control_variate.state_dict()[k] = (original_model.state_dict()[k] - self.model.state_dict()[k].to('cpu')) / global_step

        return global_step, tr_loss / global_step, new_local_control_variate
    ...
    def build_optimizer(self, model, iteration_in_total):
        ...
        #if self.args.fl_algorithm == "FedOPT" or self.args.fl_algorithm == "" or self.args.client_optimizer == 'adam':
        if self.args.client_optimizer == 'adam':
            optimizer = AdamW(model.parameters(), lr=self.args.learning_rate, eps=self.args.adam_epsilon)
        else:
            optimizer = SGD(model.parameters(), lr=self.args.learning_rate, momentum=self.args.client_momentum)
        scheduler = get_linear_schedule_with_warmup(
            optimizer, num_warmup_steps=self.args.warmup_steps, num_training_steps=iteration_in_total
        )
        print("Client Optimizer: ", optimizer)
