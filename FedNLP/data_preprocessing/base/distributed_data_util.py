...
if __name__ == '__main__':
    ...
    os.makedirs('~/fednlp_data/distributed/{}_distributed'.format(args.dataset), exist_ok=True)

    info_file = '~/fednlp_data/distributed/{}_distributed/info.json'.format(args.dataset)
    info = vars(args)
    with open(info_file, 'w') as f:
            json.dump(info, f)

    for worker_idx in tqdm.tqdm(range(worker_num)):
        worker_file = '~/fednlp_data/distributed/{}_distributed/{}.json'.format(args.dataset, worker_idx + 1)
        with open(worker_file, 'w') as f:
            json.dump(worker_list[worker_idx], f)

    # source vocab
    vocab_file = '~/fednlp_data/distributed/{}_distributed/vocab.json'.format(args.dataset)
