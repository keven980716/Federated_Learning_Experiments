...
Also, please check the arguments in ``main_fedopt.py`` to switch the server optimizer between FedAvgM and FedAdam.

An example to run the experiment (assume using FedAvgM) can be

```bash
CUDA_VISIBLE_DEVICES=0,1 sh run_fedopt_distributed_pytorch.sh 100 10 resnet56 hetero 200 20 64 0.1 cifar10 "./../../../data/cifar10" sgd 0 0.2
```
