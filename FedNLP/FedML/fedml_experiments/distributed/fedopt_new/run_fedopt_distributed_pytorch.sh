...
mpirun -np $PROCESS_NUM -hostfile ./mpi_host_file python3 ./main_fedopt.py \
  ...
  --comm_round $ROUND \
  --epochs $EPOCH \
  --client_optimizer $CLIENT_OPTIMIZER --server_optimizer sgd  \
  --batch_size $BATCH_SIZE \
  --lr $LR --client_momentum 0.9 \
  --ci $CI \
  --server_lr $SLR --server_momentum 0.9 \
...
# sh run_fedavg_distributed_pytorch.sh 100 10 resnet56 hetero 200 20 64 0.1 cifar10 "./../../../data/cifar10" sgd 0 1.0
