...
def get_dataloader_mnist_auxiliary(datadir, data_points_dict, bs):
    dl_obj = MNIST_truncated
    ...
    new_X = torch.tensor(np.array(data_points_dict['X'].copy()))
    new_y = torch.tensor(np.array(data_points_dict['y'].copy()))
    train_ds.data = new_X
...
def label_skew_process(dataset, datadir, partition, n_nets, alpha,
                       imbalance_version, imbalanced_ratio, minority_classes_list,
    ...
    for i in range(n_train):
        if y_train[i].item() in id_per_class:
            id_per_class[y_train[i].item()].append(i)
        else:
            id_per_class[y_train[i].item()] = [i]
    ...
    if need_server_auxiliary_data:
        ...
        for c in id_per_class:
            ...
            for j in range(server_auxiliary_data_per_class):
                auxiliary_data_server_X.append(X_train[id_per_class[c][j]].detach().numpy())
                auxiliary_data_server_y.append(y_train[id_per_class[c][j]].detach().numpy())
    ...
    if imbalance_version == 'binary':
        ...
        print('not implemented imbalance version')
        pass
    available_ids = []
    for c in id_per_class:
        for id in id_per_class[c]:
            available_ids.append(id)
            new_X_train.append(X_train[id].detach().numpy())
            new_y_train.append(y_train[id].detach().numpy())
    X_train = torch.tensor(np.array(new_X_train))
    y_train = torch.tensor(np.array(new_y_train))
    print(X_train.shape)
    print(y_train.shape)
    n_train = X_train.shape[0]
...
def load_partition_data_mnist(dataset, data_dir, partition_method, partition_alpha, imbalance_version, imbalanced_ratio,
    ...
    for client_idx in range(client_number):
        ...
        new_X, new_y = [], []
        for id in dataidxs:
            new_X.append(X_train[id].detach().numpy())
            new_y.append(y_train[id].detach().numpy())
        new_X = torch.tensor(np.array(new_X.copy()))
        new_y = torch.tensor(np.array(new_y.copy()))
    ...
    if auxiliary_data_server is not None:
        ...
        for id in auxiliary_data_server:
            aux_data_dl = get_dataloader_mnist_auxiliary(data_dir, auxiliary_data_server[id], batch_size)
