...
def load_partition_data_federated_emnist(dataset, data_dir, batch_size=DEFAULT_BATCH_SIZE, need_server_auxiliary_data=False,
    ...
    with h5py.File(train_file_path, 'r') as train_h5, h5py.File(test_file_path, 'r') as test_h5:
        ...
        client_ids_train = list(train_h5[_EXAMPLE].keys())
        client_ids_test = list(test_h5[_EXAMPLE].keys())
    # print(len(client_ids_train), len(client_ids_test))

    ### get auxiliary data
    print("Get auxiliary data....")
    ...
    # load data in numpy format from h5 file
    train_x = np.vstack([train_h5[_EXAMPLE][client_id][_IMGAE][()] for client_id in train_ids])
    train_y = np.hstack([train_h5[_EXAMPLE][client_id][_LABEL][()] for client_id in train_ids]).squeeze()
    test_x = np.vstack([test_h5[_EXAMPLE][client_id][_IMGAE][()] for client_id in test_ids])
    test_y = np.hstack([test_h5[_EXAMPLE][client_id][_LABEL][()] for client_id in test_ids]).squeeze()
    train_h5.close()
    test_h5.close()

    #### for generate balanced test data
    test_num_per_class = {}
    for i in range(len(test_y)):
        if test_y[i] in test_num_per_class:
            if len(test_num_per_class[test_y[i]]) < 200:
                test_num_per_class[test_y[i]].append(i)
        else:
            test_num_per_class[test_y[i]] = [i]
    
    new_test_x = []
    new_test_y = []
    for i in range(len(test_y)):
        if i in test_num_per_class[test_y[i]]:
            new_test_x.append(test_x[i])
            new_test_y.append(test_y[i])

    new_test_x = np.array(new_test_x)
    new_test_y = np.array(new_test_y).squeeze()
    ...
    for i in range(n_train):
        if train_y[i] in id_per_class:
            id_per_class[train_y[i]].append(i)
        else:
            id_per_class[train_y[i]] = [i]
    print(id_per_class.keys())
    if need_server_auxiliary_data:
        ...
        for c in id_per_class:
            ...
            for j in range(server_auxiliary_data_per_class):
                auxiliary_data_server_X.append(train_x[id_per_class[c][j]].copy())
                auxiliary_data_server_y.append(train_y[id_per_class[c][j]].copy())
            auxiliary_data_server[c] = {'X': np.array(auxiliary_data_server_X.copy()), 'y': np.array(auxiliary_data_server_y.copy()).squeeze()}
        # important!!
    ...
    if auxiliary_data_server is not None:
        ...
        for id in auxiliary_data_server:
            aux_ds = data.TensorDataset(torch.tensor(auxiliary_data_server[id]['X']), torch.tensor(auxiliary_data_server[id]['y'], dtype=torch.long))
            aux_data_dl = data.DataLoader(dataset=aux_ds,
                               batch_size=batch_size,
                               shuffle=True,
                               drop_last=False)
            auxiliary_data_server_dl[id] = aux_data_dl
        
    print("Auxiliary data processing done...")
    ...
    #### for getting balanced test data
    bal_test_ds = data.TensorDataset(torch.tensor(new_test_x), torch.tensor(new_test_y, dtype=torch.long))
    test_data_global = data.DataLoader(
                bal_test_ds,
                batch_size=batch_size, shuffle=True, drop_last=False)
    test_data_num = len(test_data_global.dataset)
