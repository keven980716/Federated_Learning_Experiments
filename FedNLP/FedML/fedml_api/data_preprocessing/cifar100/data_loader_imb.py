...
def label_skew_process(dataset, datadir, partition, n_nets, alpha, imbalance_version,
    ...
    if partition == "homo":
        ...
        label_vocab = {i: i for i in range(100)}
...
def get_dataloader_CIFAR100_auxiliary(datadir, data_points_dict, bs):
    dl_obj = CIFAR100_truncated
...
def load_partition_data_cifar100(dataset, data_dir, partition_method, partition_alpha, imbalance_version, imbalanced_ratio,
    ...
    if auxiliary_data_server is not None:
        if not retrain_server_classifier:
            ...
            for id in auxiliary_data_server:
                aux_data_dl = get_dataloader_CIFAR100_auxiliary(data_dir, auxiliary_data_server[id], batch_size)
                auxiliary_data_server_dl[id] = aux_data_dl
        else:
            auxiliary_data_server_dl = get_dataloader_CIFAR100_auxiliary(data_dir, auxiliary_data_server, batch_size)
