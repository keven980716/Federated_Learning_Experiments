...
class SCAFFOLDAggregator(object):
    ...
    def __init__(self, train_global, test_global, all_train_data_num,
        ...
        ###
        self.cumulative_local_control_variate_dict = dict()
        self.current_local_control_variate_dict = dict()
    ...
    def add_local_trained_result(self, index, model_params, sample_num, local_control_variate, client_index):
        logging.info("add_model. index = %d" % index)
        self.model_dict[index] = model_params
        self.sample_num_dict[index] = sample_num
        ###
        self.current_local_control_variate_dict[index] = local_control_variate
        # str -> int
    ...
    def aggregate(self):
        ...
        for idx in range(self.worker_num):
            ...
            model_list.append((self.sample_num_dict[idx], self.model_dict[idx],
                               self.current_local_control_variate_dict[idx], self.current_client_index_dict[idx]))
            ###
        ...
        if self.update_step == 0:
            self.global_control_variate = copy.deepcopy(self.trainer.model.to('cpu'))
            for k in self.global_control_variate.state_dict().keys():
                if 'weight' in k or 'bias' in k:
                    self.global_control_variate.state_dict()[k].data -= self.global_control_variate.state_dict()[k].data
            for i in range(self.args.client_num_in_total):
                self.cumulative_local_control_variate_dict[i] = 0

        for k in averaged_params.keys():
            for i in range(0, len(model_list)):
                local_sample_number, local_model_params, local_control_variate, client_index = model_list[i]
                ...
                if 'weight' in k or 'bias' in k:
                    if self.update_step == 0:
                        if i == 0:
                            # if self.cumulative_local_control_variate_dict[client_index] == 0:
                            # print(k, self.global_control_variate.state_dict()[k].norm(), local_control_variate.state_dict()[k].norm())
                            # self.global_control_variate.state_dict()[k].data -= self.global_control_variate.state_dict()[k].data
                            self.global_control_variate.state_dict()[k].data += local_control_variate.state_dict()[k].data * w * (
                                            self.args.client_num_per_round / self.args.client_num_in_total)
                            # print(self.global_control_variate.state_dict()[k].norm())

                            # else:
                            #     self.global_control_variate.state_dict()[k] = (local_control_variate.state_dict()[k].data - self.cumulative_local_control_variate_dict[client_index].state_dict()[k].data) * w * (
                            #                 self.args.client_num_per_round / self.args.client_num_in_total)
                        else:
                            # if self.cumulative_local_control_variate_dict[client_index] == 0:
                            self.global_control_variate.state_dict()[k].data += local_control_variate.state_dict()[k].data * w * (
                                        self.args.client_num_per_round / self.args.client_num_in_total)
                            # else:
                            #     self.global_control_variate.state_dict()[k] += (local_control_variate.state_dict()[k] - self.cumulative_local_control_variate_dict[client_index].state_dict()[k]) * w * (
                            #             self.args.client_num_per_round / self.args.client_num_in_total)
                    else:
                        if self.cumulative_local_control_variate_dict[client_index] == 0:
                            self.global_control_variate.state_dict()[k].data += local_control_variate.state_dict()[k].data * w * (
                            ...
                            self.global_control_variate.state_dict()[k].data += (local_control_variate.state_dict()[k].data - self.cumulative_local_control_variate_dict[client_index].state_dict()[k].data) * w * (
                                                                      self.args.client_num_per_round / self.args.client_num_in_total)


        # update local control variate
        for i in range(0, len(model_list)):
            _, _, local_control_variate, client_index = model_list[i]
            self.cumulative_local_control_variate_dict[client_index] = local_control_variate
        ...
        return averaged_params, self.global_control_variate, self.cumulative_local_control_variate_dict
