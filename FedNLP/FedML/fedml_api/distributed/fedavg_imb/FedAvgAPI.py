...
from ...standalone.fedavg.my_model_trainer_imb_classification import MyModelTrainer as MyModelTrainerCLS
from ...standalone.fedavg.my_model_trainer_imb_classification_decoupling import MyModelTrainer as MyModelTrainerCLSDecoupling
...
def FedML_FedAvg_distributed(process_id, worker_number, device, comm, model, train_data_num, train_data_global, test_data_global,
    ...
    if process_id == 0:
        init_server(args, device, comm, process_id, worker_number, model, train_data_num, train_data_global,
                    ...
                    model_trainer, preprocessed_sampling_lists, auxiliary_data_server=auxiliary_data_server)
    else:
...
def init_server(args, device, comm, rank, size, model, train_data_num, train_data_global, test_data_global,
    ...
    if model_trainer is None:
        if args.dataset == "stackoverflow_lr":
            ...
            if args.decoupled_training or args.use_creff:
                model_trainer = MyModelTrainerCLSDecoupling(model)
            else:
    ...
    aggregator = FedAVGAggregator(train_data_global, test_data_global, train_data_num,
                                  ...
                                  worker_num, device, args, model_trainer, auxiliary_data_server=auxiliary_data_server)
