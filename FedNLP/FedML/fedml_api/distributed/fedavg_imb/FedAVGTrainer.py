...
class FedAVGTrainer(object):
    ...
    def train(self, round_idx=None, alpha=None, prototypes=None, balanced_gradients=None,
              ba_params=None, creff_retrained_params=None):
        if round_idx is not None:
            self.args.round_idx = round_idx
        if not (self.args.decoupled_training or self.args.use_creff):
            local_training_loss = self.trainer.train(self.train_local, self.device, self.args, alpha=alpha)
        else:
            local_training_loss, balanced_gradients = self.trainer.train(self.train_local, self.device, self.args, alpha=alpha,
                                                                         prototypes=prototypes, balanced_gradients=balanced_gradients,
                                                                         ba_params=ba_params,
        ...
        return weights, self.local_sample_number, local_training_loss, balanced_gradients
