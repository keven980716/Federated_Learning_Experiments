...
class CNN_DropOut(torch.nn.Module):
    ...
    def __init__(self, only_digits=True, return_final_hidden_states=False):
    ...
    def forward(self, x):
        ...
        #x = self.linear_1(x)
        x = self.linear_1(x)
        final_states = self.relu(x)
        #x = self.relu(x)
        x = self.dropout_2(final_states)
        x = self.linear_2(x)
        #x = self.softmax(self.linear_2(x))
        if self.return_final_hidden_states:
            return x, final_states
        else:
            return x


class CNN_DropOut_ba(torch.nn.Module):
    """
    Recommended model by "Adaptive Federated Optimization" (https://arxiv.org/pdf/2003.00295.pdf)
    Used for EMNIST experiments.
    When `only_digits=True`, the summary of returned model is
    ...
    def __init__(self, only_digits=True, return_final_hidden_states=False):
        super(CNN_DropOut_ba, self).__init__()
        self.conv2d_1 = torch.nn.Conv2d(1, 32, kernel_size=3)
        self.max_pooling = nn.MaxPool2d(2, stride=2)
        self.conv2d_2 = torch.nn.Conv2d(32, 64, kernel_size=3)
        self.dropout_1 = nn.Dropout(0.25)
        ...
        #self.softmax = nn.Softmax(dim=1)
        self.return_final_hidden_states = return_final_hidden_states
        self.bias_attractor = nn.Linear(128, 10 if only_digits else 62)
    ...
    def forward(self, x, use_ba=False, only_linear=False):
        ...
        if not use_ba:
            if not only_linear:
                x = self.linear_2(x_out)
            else:
                x = self.linear_2(x_out.detach())
        else:
            x = self.linear_2(x_out)
            # x_ba = self.dropout_2(x_out)
            x_ba = self.bias_attractor(x_out)
            x_add = x + x_ba 
        if self.return_final_hidden_states:
            if not use_ba: 
                return x, x_out
